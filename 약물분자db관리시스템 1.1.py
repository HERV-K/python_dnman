...
def openneowindow4():
    ...
    def w4_maketable():
        ...
        for a in range(1, pagenum +1):
            ...
            pagelist[a-1] = Image.new("RGBA", (int(combox_w4_indimgsize.get()[0:combox_w4_indimgsize.get().rfind("x")]) * columnnum, int(combox_w4_indimgsize.get()[combox_w4_indimgsize.get().rfind("x") + 1:])*rownum), (0, 0, 0, 0))
            print(f"페이지번호 : {a}")
    ...
    layoutlist = ["5x6","4x5","4x4", "5x5","6x6"]
...
def openneowindow5():
    ...
    btn_w5_4 = Button(neowindow5, width=20, text="예비4")
    btn_w5_4.grid(row=2, column=2, sticky=W + E + N + S)

    btn_w5_5 = Button(neowindow5, width=20, text="예비5")
    btn_w5_5.grid(row=2, column=3, sticky=W + E + N + S)

    btn_w5_6 = Button(neowindow5, width=20, text="예비6")
